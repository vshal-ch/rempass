...
const signupFieds = document.querySelectorAll(".input-signup");
const signupBtn = document.querySelector('.signup-btn')
const loginFieds = document.querySelectorAll(".input-login");
...
function validateInput() {
  ...
  if(this.classList.contains('input-signup')){
    if(signupFieds[2].value!=''){
      checkSignup();
    }
  }
  if(this.classList.contains('input-login')){
    if(loginFieds[1].value!=''){
      checkLogin();
    }
  }
  else if(this.classList.contains('input-no-login')){
...
function handleFocusOut() {
  ...
  else if(this.classList.contains('input-login')){
      checkLogin();
  }
  else if(this.classList.contains('input-no-login')){
      checkSubmit();
...
function checkSignup(){
  let c = 0;
    signupFieds.forEach((input) => {
      if (validateHelper(input.value, input.dataset.pattern).validation) {
        c++;
      }
    });
    if (c == 3) {
      toggleDisable(signupBtn, false);
    } else {
      toggleDisable(signupBtn, true);
    }
}

function checkLogin(){
  let c = 0;
    loginFieds.forEach((input) => {
      if (validateHelper(input.value, input.dataset.pattern).validation) {
        c++;
      }
    });
    if (c == 2) {
      toggleDisable(loginBtn, false);
    } else {
      toggleDisable(loginBtn, true);
    }
}
...
signupFieds.forEach((field) => {
  field.addEventListener("input", validateInput);
  field.addEventListener("focusout", handleFocusOut);
});
loginFieds.forEach((field) => {
  field.addEventListener("input", validateInput);
  field.addEventListener("focusout", handleFocusOut);
});
